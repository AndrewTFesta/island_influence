...
def create_agent_policy(observation_space, action_space, learner):
    ...
    # action_size = 2
    obs_size = np.prod(observation_space)
...
def create_base_env(
    ...
    harvesters = [
        NeuralNetwork(n_inputs=state_size, n_outputs=action_size, n_hidden=n_hidden)
    ...
    # normalize_rewards
    env_func = partial(HarvestEnv, actors={'excavator': excavators, 'harvester': harvesters},
                       num_harvesters=num_harvesters, num_excavators=num_excavators, num_obstacles=num_obstacles, num_pois=num_pois,
                       location_funcs=location_funcs, max_steps=max_steps, delta_time=delta_time, collision_penalty_scalar=collision_penalty_scalar,
                       reward_type=reward_type, normalize_rewards=normalize_rewards, render_mode=render_mode, save_dir=save_dir)
    return env_func
